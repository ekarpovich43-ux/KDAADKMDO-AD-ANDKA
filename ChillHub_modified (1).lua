...
local CreditsGroup = Tabs.Credits:AddLeftGroupbox("Credits")

CreditsGroup:AddLabel({
	Text = "i dont know (mrbeanlololo) - developer",
	DoesWrap = true,
	Size = 14,
})
CreditsGroup:AddDivider()
CreditsGroup:AddLabel({
	Text = "Thank you very much to the people who regularly use our scripts, thank you to our testers, who are ready to test our scripts even in difficult times, a huge thank you to the developers who always make scripts for us. You are all not just our regular users, you are a family that will grow every time. If you have questions, discuss suggestions for our scripts, or report bugs that are present in our scripts, please join our Discord server using the button below!",
	DoesWrap = true,
	Size = 13,
})
CreditsGroup:AddDivider()

local DiscordLink = "https://discord.gg/RS7CHduD"
...
CreditsGroup:AddButton({
	...
	Func = function()
		...
		if not copied and toclipboard then
			local ok = pcall(function() toclipboard(DiscordLink) end)
			copied = copied or ok
		end
		if not copied and syn and syn.write_clipboard then
			local ok = pcall(function() syn.write_clipboard(DiscordLink) end)
			copied = copied or ok
		end
		if not copied and Clipboard and Clipboard.set then
			local ok = pcall(function() Clipboard.set(DiscordLink) end)
			copied = copied or ok
		end
		Library:Notify({
			Title = "Chill Hub",
			Description = copied and "Discord link copied!" or "Clipboard is not supported by your executor.",
			Time = 4,
